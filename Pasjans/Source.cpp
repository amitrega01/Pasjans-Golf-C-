		...
		if (_kbhit()) {
			...
			case 's': {
				if (kol[aktywnaKolumna].wierzcholek != nullptr)
					if (kol[aktywnaKolumna].getValue() + 1 == gracza.getValue() || kol[aktywnaKolumna].getValue() - 1 == gracza.getValue()) { score += 110;gracza.push(kol[aktywnaKolumna].pop()->karta);if (b)  Beep(800, 200);
				}
					else {
						if (b) Beep(500, 200);
					}
